...
function Register(props) {
  ...
  const classes=useStyles();
    const initialUserState={ username: "", password: "",firstName:"",lastName:"",email: "",roleId:1};
    const [newUser,setnewUser]=useState(initialUserState);
 
    const handleChange=(event)=>{ setnewUser({...newUser,[event.target.name]:event.target.value}) }
    ...
    const reset=()=>{setnewUser(initialUserState)}
        return (
